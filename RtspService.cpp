...
static int init_mmap(int fd)
{
	...
	for(nBuffer = 0; nBuffer < nReqBufs.count; nBuffer++)
	{
		// stand for a frame
		...
		usr_buf[nBuffer].start = (char *)mmap(NULL, buf.length, PROT_READ | 
...
static int init_camera(int fd)
{
	...
	tv_fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV;  // V4L2_PIX_FMT_YYUV | V4L2_PIX_FMT_RGB24
...
static void open_camera(void)
{
	...
	CameraDeviceFd = open(DEVICE_VIDEO, O_RDWR, 0);
	if(CameraDeviceFd < 0)
	{
